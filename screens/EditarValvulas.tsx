...
const EditValvulas = () => {
  ...
  const [selected, setSelected] = useState<Valvula | null>(null);
  ...
  const handleSave = () => {
    ...
    if (editMode && selected) {
      db.transaction(tx => {
        tx.executeSql(
          'UPDATE valvulas_controle SET tipo=?, nome=?, descricao=? WHERE id=?;',
      ...
      db.transaction(tx => {
        tx.executeSql(
          'INSERT INTO valvulas_controle (tipo, nome, descricao) VALUES (?, ?, ?);',
  ...
  const handleDelete = () => {
    ...
    Alert.alert(
      'Excluir',
      `Excluir a válvula ${selected.nome}?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: () => {
            db.transaction(tx => {
              tx.executeSql(
                'DELETE FROM valvulas_controle WHERE id=?;',
                [selected.id],
                () => {
                  fetchValvulas();
                  setSelected(null);
        ...
        <View style={styles.modalBg}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>
              {editMode ? 'Editar Válvula' : 'Nova Válvula'}
            </Text>
            ...
            <TextInput
              placeholder="Ex: Válvula de controle da linha principal"
              style={styles.input}
