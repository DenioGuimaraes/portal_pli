...
class U1620PsvModel extends Model
{
    ...
    public function u1620psvGetAll() {
    ...
    public function u1620psvGetById($id) {
        $sql = "SELECT * FROM u1620_psv WHERE id = " . intval($id);
        $result = $this->db->query($sql);
        return $result ? $result->fetch_assoc() : null;
    }

    // 🔹 Cria uma nova PSV
    public function u1620psvCreate($data) {
    ...
    // 🔹 Atualiza uma PSV
    public function u1620psvUpdate($id, $data) {
        $tag = $this->db->real_escape_string($data['tag']);
        $localizacao = $this->db->real_escape_string($data['localizacao']);
        $fluido = $this->db->real_escape_string($data['fluido']);
        $alivio = $this->db->real_escape_string($data['alivio']);
    ...
    // 🔹 Deleta uma PSV
    public function u1620psvDelete($id) {
        $sql = "DELETE FROM u1620_psv WHERE id = " . intval($id);
        return $this->db->query($sql);
    }
}
